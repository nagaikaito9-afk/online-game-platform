...
class SlimeEngine {
  ...
  feedSlimes() {
    ...
    this.activeSlimes.forEach(slime => {
      slime.targetRadius += 15; // 巨大化！
      slime.atk = Math.floor(slime.atk * 1.15);
      slime.maxHp = Math.floor(slime.maxHp * 1.15);
  ...
  splitSlime() {
    ...
    if (largest.radius < 35) {
      return { success: false, msg: 'スライムが小さすぎてまだ分裂できません！もっと巨大化させてね！' };
    }
  ...
  // 💕 繁殖 (Breed Slimes)
  breedSlimes() {
    if (this.activeSlimes.length < 2) {
      return { success: false, msg: '繁殖には2体以上のスライムが必要です！' };
    }

    if (window.slimeAudioEngine) window.slimeAudioEngine.playSE('breed');

    const p1 = this.activeSlimes[0];
    const p2 = this.activeSlimes[1];

    // ランダムな子供スライムの生誕
    const childId = Math.floor(Math.random() * 50) + 1; // コモン〜アンコモンから誕生
    ...
    const child = {
      ...
      radius: 30,
      targetRadius: 30,
      atk: Math.floor((p1.atk + p2.atk) * 0.6),
      hp: Math.floor((p1.hp + p2.hp) * 0.6),
      maxHp: Math.floor((p1.hp + p2.hp) * 0.6),
      wobble: 1.2
    };

    this.activeSlimes.push(child);
    return { success: true, msg: `💕 【${child.name}】が繁殖によって誕生しました！` };
  }
    ...
    this.activeSlimes.forEach(slime => {
      ...
      for (let i = this.foods.length - 1; i >= 0; i--) {
        ...
        if (dx * dx + dy * dy <= (slime.radius + f.radius) * (slime.radius + f.radius)) {
          slime.targetRadius += 4; // 餌を食べて大きくなる！
          slime.wobble = 0.6;
  ...
  render() {
    ...
    this.activeSlimes.forEach(s => {
      ...
      // プニプニ変形 (Soft body Elastic Wobble)
